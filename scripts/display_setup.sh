...
if xrandr | grep "$EXTERNAL_DISPLAY connected"; then
    ...
    xrandr --output $INTERNAL_DISPLAY --off --output $EXTERNAL_DISPLAY --mode 3440x1440 --dpi 100 --rate 75 --primary
else
    # If external monitor is not connected, use the internal display
    xrandr --output $INTERNAL_DISPLAY --auto --primary --output $EXTERNAL_DISPLAY --off --dpi 192
fi
